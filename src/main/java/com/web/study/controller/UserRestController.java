...
import com.web.study.controller.dto.UserAdditionDto;
...
public class UserRestController {
	...
	public ResponseEntity<? extends ResponseDto> userAddition(@RequestBody UserAdditionDto userAdditionDto){
		Map<Integer, UserAdditionDto> userMap = UserStore.userMap; 
		int maxKey = 0;  
		if(!userMap.keySet().isEmpty()) { //등록된것이 있으면 실행
	...
	public ResponseEntity<? extends ResponseDto> getUser(@PathVariable int id){
		...
		//만약에 해당 id가 존재하지 않으면 ErrorResponse를 응답으로 해준다. errorMessage = 존재하지 않는 id입니다.
		Map<Integer, UserAdditionDto> userMap = UserStore.userMap;
